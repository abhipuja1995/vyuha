...
async def upload_call_recording(
    ...
    # Parse any manually provided transcript
    try:
    ...
    # Read audio bytes once (needed for both saving and optional STT)
    audio_bytes = await audio_file.read()
    ...
    # Auto-transcribe via Ollama if no manual transcript and STT is configured
    stt_used = False
    ...
    record = FailedCallRecord(
        ...
        failure_signals=pre_signals,
        metadata={
            "source": "manual_upload",
            "original_filename": audio_file.filename,
            **({"stt": "ollama"} if stt_used else {}),
        },
    )
    ...
    if not transcript:
        reason = "Audio saved."
        if stt_error:
            reason += f" Ollama transcription failed: {stt_error}"
        else:
            reason += " Add a transcript and re-submit to generate a test case."
            if not settings.ollama_url:
                reason += " (Set OLLAMA_URL to enable auto-transcription.)"
        return {
            "ingested": False,
            "call_id": effective_call_id,
            "audio_path": str(audio_path),
